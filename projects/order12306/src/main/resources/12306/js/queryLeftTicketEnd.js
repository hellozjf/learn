...
(function() {
	...
	function a7(cL) {
		if (cL && cL.length > 0) {
			var cR = [];
			var cM = [];
			for (var cN = 0, cT = cL.length; cN < cT; cN++) {
				var cP = cL[cN];
	...
	function cs(cU, c8, c4, cZ, c5, cW, cS, cV) {
		...
		if ("无" == c8 && "WZ_" != c4 && "QT_" != c4 && c0) {
			...
			if (c1 != "" && cN.houbu_seat_limit && cN.houbu_seat_limit.indexOf(c1) > -1) {
				...
				c6 = "0";
				cX = "当前" + cN.station_train_code + "车次" + O(c4.replace("_", "")) + "席别提交的候补订单较多，可更换车次、席别或稍后重试。"
			} else {
			...
			success: function(cP) {
				...
				if (cP.status) {
					...
					if (train_tour_flag == "gc" && "Y" == isDwTicketResign) {
						var cX = [];
						for (var cQ = 0, cY = cP.data.length; cQ < cY; cQ++) {
	...
	function x(cS, cO) {
		...
		if (cO == "add") {
			$(".buy-cart").show();
			var cP = $(".cart-tlist li").length;
			var cQ = "";
			cQ += '<li tdate="' + cN[0] + '" hbid="' + cS + '">';
			cQ += '<div class="card-num">' + (cP + 1) + "</div>";
			cQ += '<div class="cart-tlist-date">' + cN[0] + "</div>";
			cQ += "<div>" + cN[6] + " → " + cN[8] + "</div>";
			cQ += "<div><strong>" + cN[10] + "</strong> " + seatTypeForHB[cN[4].split("_")[0]].split("_")[1] + "</div>";
			cQ += '<div name="suc_rate" hbid="' + cS + '" class="cart-tlist-tool">';
			cQ += '<a href="javascript:void(0)" onclick="deleteHB(this)" class="del">删除</a>';
			cQ += "</div>";
			cQ += "</li>";
			$(".cart-tlist").append(cQ);
			var cM = seatTypeForHB[cS.split("#")[4].split("_")[0]].split("_")[0];
			var cR = cS.split("#")[9] + "#" + cM;
			bO(cR, cS)
		} else {
			$('li[hbid="' + cS + '"]').remove()
		}
		...
		$.ajax({
			...
			success: function(cN) {
				...
				if (cO && cO.flag) {
					var cP = cO.flag[0];
					if (cP.level == 3) {
						$('div[name="suc_rate"][hbid="' + cM + '"]').prepend('<span class="odds-small">' + cP.info + "</span>")
					} else {
						if (cP.level == 2) {
							$('div[name="suc_rate"][hbid="' + cM + '"]').prepend('<span class="odds-mid">' + cP.info + "</span>")
						} else {
							if (cP.level == 1) {
								$('div[name="suc_rate"][hbid="' + cM + '"]').prepend('<span class="odds-large">' + cP.info + "</span>")
							}
						}
					}
				}
	...
	function U() {
		$(".cart-hd span").html($(".cart-tlist li").length);
		for (var cM = 0; cM < $(".cart-tlist li").length; cM++) {
			$(".cart-tlist li").eq(cM).find("div").eq(0).html(cM + 1)
		}
		if ($(".cart-tlist li").length == 0) {
			$(".cart-bd").hide()
		} else {
		...
		renderPassenger: function(c3) {
			...
			$("#buyer-list li").click(function() {
				...
				if (!$(this).hasClass("cur")) {
					...
					$.each($("#setion_postion span"), function(c9, da) {
						if (c8 == $(da).attr("name")) {
							$(da).remove();
							$.removePasseanger(c8);
							return
				...
				$.ajax({
					...
					success: function(cS) {
						if (cS.status) {
							...
							if (train_tour_flag == "gc" && "Y" == isDwTicketResign) {
								var cZ = [];
								for (var cT = 0, c0 = cS.data.length; cT < c0; cT++) {
				...
				success: function(b) {
					...
					if (b.result_code == 0) {
						...
						if (b.result_code == 91) {
							var c = b.newapptk || b.apptk;
							apptk_tmp_control = c;
							$("#login_control_mobile").html("请用尾号" + b.mobile + "手机号发短信9999到12306。");
...
jQuery.extend({
	...
	popup_show_login_error: function(a) {
		...
		if (a && a.indexOf("重新设置密码") > -1) {
			a = a.substring(0, a.indexOf("重新设置密码")) + '<a href="' + popup_baseUrl + popup_publicName + '/forgetPassword/initforgetMyPassword" style="color:#3B99FC">重新设置密码</a>' + a.substring(a.indexOf("重新设置密码") + 6)
		}
