(function() {
	...
	function Ba(a, b, c, d) {
		...
		var e = K(c);
		if (e) {
			"OBJECT" == e.nodeName ? (Y = Aa(e), na = null) : (Y = e, na = c);
	...
	function Fa() {
		...
		if (0 < a) for (var b = 0; b < a; b++) {
			...
			if (0 < n.pv[0]) {
				var f = K(c);
				if (f) if (!ka(M[b].swfVersion) || n.wk && 312 > n.wk) if (M[b].expressInstall && Da()) {
					e = {};
	...
	function Ga(a) {
		if (!a) return "";
		if (rb(a)) return a.replace(/\s/g, ""); - 1 != a.indexOf("://") && (a = a.substr(a.indexOf("://") + 3));
	...
	function aa(a) {
		for (var b = [], c = (1 << ba) - 1, d = 0; d < a.length * ba; d += ba) b[d >> 5] |= (a.charCodeAt(d / ba) & c) << d % 32;
		a = a.length * ba;
		b[a >> 5] |= 128 << a % 32;
		b[(a + 64 >>> 9 << 4) + 14] = a;
		a = 1732584193;
		...
		b = [a, c, d, e];
		a = sb ? "0123456789ABCDEF" : "0123456789abcdef";
		c = "";
	...
	function V(a, b, c) {
		...
		u.cookie = a + "\x3d" + b + "; path\x3d/;expires \x3d " + d.toGMTString() + ";domain\x3d" + Ga(r.location.host.split(":")[0])
	}
	function Ha() {
		var a = k.userAgent.toLowerCase();
		return 0 <= a.indexOf("windows phone") ? "WindowsPhone" : 0 <= a.indexOf("win") ? "Windows" : 0 <= a.indexOf("android") ? "Android" : 0 <= a.indexOf("linux") ? "Linux" : 0 <= a.indexOf("iphone") || 0 <= a.indexOf("ipad") ?
			"iOS" : 0 <= a.indexOf("mac") ? "Mac" : "Other"
	}
	function ja() {
	...
	if (!(Ia = ca)) {
		var da = Math,
			pa = {}, qa = pa.lib = {}, bb = function() {}, ea = qa.Base = {
				clone: function() {
				...
				random: function(a) {
					for (var b = [],
					c = 0; c < a; c += 4) b.push(4294967296 * da.random() | 0);
			...
			}, db = qa.BufferedBlockAlgorithm = ea.extend({
				clone: function() {
					var a = ea.clone.call(this);
	...
	for (var ca = Ia, ra = Math, sa = ca, R = sa.lib, zb = R.WordArray, ta = R.Hasher, R = sa.algo, eb = [], fb = [], ua = 2, ga = 0; 64 > ga;) {
		var W;
		a: {
			W = ua;
			for (var Ab = ra.sqrt(W), Ka = 2; Ka <= Ab; Ka++) if (!(W % Ka)) {
				W = !1;
				break a
			}
			W = !0
		}
		W && (8 > ga && (eb[ga] = Za(ra.pow(ua, .5))), fb[ga] = Za(ra.pow(ua, 1 / 3)), ga++);
		ua++
	}
	var S = [],
		R = R.SHA256 = ta.extend({
			_doReset: function() {
				this._hash = new zb.init(eb.slice(0))
			},
			clone: function() {
			...
			b) {
				for (var c = this._hash.words, d = c[0], e = c[1], f = c[2], g = c[3], l = c[4], k = c[5], n = c[6], t = c[7], h = 0; 64 > h; h++) {
					if (16 > h) S[h] = a[b + h] | 0;
						...
						var m = S[h - 15],
							p = S[h - 2];
						S[h] = ((m << 25 | m >>> 7) ^ (m << 14 | m >>> 18) ^ m >>> 3) + S[h - 7] + ((p << 15 | p >>> 17) ^ (p << 13 | p >>> 19) ^ p >>> 10) + S[h - 16]
					}
					m = t + ((l << 26 | l >>> 6) ^ (l << 21 | l >>> 11) ^ (l << 7 | l >>> 25)) + (l & k ^ ~l & n) + fb[h] + S[h];
	...
	Z.prototype = {
		...
		getCanvasFp: function() {
			...
			c.fillStyle = "#069";
			c.font = this.options.dontUseFakeFontInCanvas ? "11pt Arial" :
		...
		getHasLiedBrowser: function() {
			...
			if (("Chrome" === a || "Safari" === a ||
				"Opera" === a) && "20030107" !== b) return !0;
		...
		getScreenResolution: function(a) {
			var b;
			b = this.options.detectScreenOrientation ? y.height > y.width ? [y.height, y.width] : [y.width, y.height] : [y.width,
			y.height];
			"undefined" !== typeof b && a.push({
				key: "resolution",
				value: b
	...
	var sb = 0,
		...
		ma = !1,
		L, ya, Ta = !0,
		n, Db = "undefined" != typeof q.getElementById && "undefined" != typeof q.getElementsByTagName && "undefined" != typeof q.createElement,
		...
		ha = [0, 0, 0],
		I = null;
	if ("undefined" != typeof O.plugins && "object" == typeof O.plugins["Shockwave Flash"])!(I = O.plugins["Shockwave Flash"].description) || "undefined" != typeof O.mimeTypes && O.mimeTypes["application/x-shockwave-flash"] && !O.mimeTypes["application/x-shockwave-flash"].enabledPlugin || (ib = !0, La = !1, I = I.replace(/^.*\s+(\S+\s+\S+$)/,
		"$1"), ha[0] = parseInt(I.replace(/^(.*)\..*$/, "$1"), 10), ha[1] = parseInt(I.replace(/^.*\.(.*)\s.*$/, "$1"), 10), ha[2] = /[a-zA-Z]/.test(I) ? parseInt(I.replace(/^.*[a-zA-Z]+(.*)$/, "$1"), 10) : 0);
	else if ("undefined" != typeof J.ActiveXObject) try {
		if (I = (new ActiveXObject("ShockwaveFlash.ShockwaveFlash")).GetVariable("$version")) La = !0, I = I.split(" ")[1].split(","), ha = [parseInt(I[0], 10), parseInt(I[1], 10), parseInt(I[2], 10)]
	} catch (a) {}
	n = {
		mac: Fb,
		wk: Gb,
	...
	n.w3 && (("undefined" != typeof q.readyState &&
		"complete" == q.readyState || "undefined" == typeof q.readyState && (q.getElementsByTagName("body")[0] || q.body)) && U(), Q || ("undefined" != typeof q.addEventListener && q.addEventListener("DOMContentLoaded", U, !1), n.ie && n.win && (q.attachEvent("onreadystatechange", function b() {
	...
	X = {
		getQueryParamValue: function(a) {
			...
			if (b) {
				/\?/.test(b) && (b = b.split("?")[1]);
				if (null == a) return Sa(b);
				for (var b = b.split("\x26"), c = 0; c < b.length; c++) if (b[c].substring(0, b[c].indexOf("\x3d")) == a) return Sa(b[c].substring(b[c].indexOf("\x3d") + 1))
			}
			return ""
		},
		...
		b, c, d, e, f, g, l, k, p) {
			...
			n.w3 && !(n.wk && 312 > n.wk) && a && b && c && d && e ? (P(b, !1), Ya(function() {
				c += "";
				d += "";
				var h = {};
				...
				if (ka(e)) n = za(h, m, b), h.id == b && P(b, !0), t.success = !0, t.ref = n;
				else {
					if (f && Da()) {
						h.data = f;
						Ba(h, m, b, p);
		...
		expressInstallCallback: function() {
			if (ma) {
				...
				a && Y && (a.parentNode.replaceChild(Y, a), na && (P(na, !0), n.ie && n.win && (Y.style.display = "block")), Ca && Ca(Wa));
				ma = !1
			}
		},
		addLoadEvent: Xa,
	...
	var Hb = ["sessionStorage", "localStorage", "indexedDb", "openDatabase"];
	Array.prototype.indexOf || (Array.prototype.indexOf = function(a, b) {
		var c;
	...
	var ba = 8;
	Z.VERSION = "1.4.2";
	var Ib = "appCodeName appMinorVersion appName cpuClass onLine systemLanguage userLanguage historyList hasLiedLanguages hasLiedResolution hasLiedOs hasLiedBrowser".split(" "),
		Jb = ["scrAvailWidth", "scrAvailHeight"];
	try {
		var w = r,
			x = w.document,
			jb = w.Image,
		...
		var ia, xa, lb = {
			...
			silverlight: !1,
			domain: Ga(w.location.host.split(":")[0]),
		...
		w.evercookie = w.Evercookie = function(a) {
			...
			this._evercookie = function(a, c, d, B, l) {
				...
				if (void 0 !== d)("undefined" === typeof ia ||
					"undefined" === typeof xa) && B++ < g && H(function() {
					...
					"" === h._ec.pngData) || "undefined" === typeof xa) && B++ < g) H(function() {
					h._evercookie(a, c, d, B, l)
				}, 20);
				else {
			...
			this.evercookie_auth = function(a, c) {
				if (void 0 !== c) {
					var d = "//" + c + "@" + ub.host + l + p + b.authPath + "?name\x3d" + a,
						e = new jb;
			...
			this.evercookie_lso = function(a, b) {
				...
				f.id = "myswf";
				f.name = "myswf";
				kb.embedSWF(l + k + "/evercookie.swf", "swfcontainer", "1", "1", "9.0.0", !1, d, e, f)
			};
			this.evercookie_png = function(a, c) {
				...
				d.width = 200;
				d.height = 1;
				d && d.getContext && (e = new jb, e.style.visibility = "hidden", e.style.position = "absolute", void 0 !== c ? x.cookie = b.pngCookieName + "\x3d" + c + "; path\x3d/; domain\x3d" + n : (h._ec.pngData = void 0, f = d.getContext("2d"), g = this.getFromStr(b.pngCookieName, x.cookie), x.cookie = b.pngCookieName + "\x3d; expires\x3dMon, 20 Sep 2010 00:00:00 UTC; path\x3d/; domain\x3d" + n, e.onload = function() {
			...
			this.getHost = function() {
				return Ga(w.location.host.split(":")[0])
			};
			...
			this._getRGB = function(a, b) {
				...
				var c;
				if (x.defaultView) {
					if (null == x.defaultView.getComputedStyle(q,
					null)) return -1;
	...
	ja.prototype = {
			...
			this.ec.get("RAIL_OkLJUJ", function(a) {
				...
				c.getDfpMoreInfo(function() {
					if (!(9E5 < G("RAIL_EXPIRATION") - (new Date).getTime() & null != G("RAIL_DEVICEID") & void 0 != G("RAIL_DEVICEID") & !c.NeedUpdate())) {
						for (var a = "", e = "", l = c.getpackStr(b), k = [], n = [], q = [], h = [], m = 0; 
							m < l.length; 
							m++) 
								"new" != l[m].value && -1 == Ib.indexOf(l[m].key) && (-1 != Hb.indexOf(l[m].key) ? n.push(l[m]) : -1 != Jb.indexOf(l[m].key) ? q.push(l[m]) : -1 != Cb.indexOf(l[m].key) ? h.push(l[m]) : k.push(l[m]));
						l = "";
						...
						ab.getJSON("https://kyfw.12306.cn/otn/HttpZF/logdevice" + ("?algID\x3dQWuppEjqYg\x26hashCode\x3d" + e + a), null, function(a) {
							var b = JSON.parse(a);
							void 0 != mb && mb.postMessage(a, r.parent);
							for (var d in b) "dfp" == d ? G("RAIL_DEVICEID") != b[d] && (V("RAIL_DEVICEID", b[d], 1E3), c.deviceEc.set("RAIL_DEVICEID", b[d])) : "exp" == d ? V("RAIL_EXPIRATION", b[d], 1E3) : "cookieCode" == d && (c.ec.set("RAIL_OkLJUJ", b[d]),
							V("RAIL_OkLJUJ", "", 0))
						})
					}
				})
		...
		getJsFonts: function(a) {
			return new p("jsFonts", aa(a.replace(RegExp(",", "gm"), "#")))
		},
		...
		getScrAvailWidth: function() {
			return new p("scrAvailWidth",
			r.screen.availWidth.toString())
		},
		getUserAgent: function() {
			var a = k.userAgent,
				a = a.replace(/\&|\+/g, "");
		...
		getAppCodeName: function() {
			return new p("appCodeName",
			k.appCodeName.toString())
		},
		...
		hashAlg: function(a, b, c) {
			...
			for (var d = 0; d < a.length; d++) {
				...
				"" !== f && (c += e + f, b += "\x26" + (void 0 == hb[e] ? e : hb[e]) + "\x3d" + f)
			}
			a = Qa(c);
			a = Ra(a);
			a = Qa(a);
		...
		getMimeTypes: function() {
			...
			return new p("mimeTypes", aa(b.substr(0, b.length - 1)))
		},
		NeedUpdate: function() {
			V("fp_ver", "4.6.1", 0);
			V("BSFIT_OKLJUJ", "", 0);
	...
	u.addEventListener ? u.addEventListener("DOMContentLoaded", function b() {
		...
		nb || "interactive" != u.readyState && "complete" != u.readyState || (u.detachEvent("onreadystatechange", c), Pa(), nb = !0)
	})
})();