(function(window) {
	...
	touclick["touclick_fun_checkRandCode"] = function(obj) {
		var check = false,
			randCode = obj.value,
			rand = "sjrand",
			touclick = TouClick.get("touclick-" + obj.id),
			ispassenger = 0;
	...
	touclick["touclick_fun_showSuc"] = function() {
		return $('');
	};
	...
	css["login_"] = {
		"float": {
			...
			"hook": function(touclickObject, $target) {
				...
				$(touclickObject.getDom()).parent('.captchaButton').css({
					'z-index': '100'
				});
				return function() {};
			}
		...
		"inside": {
			...
			"hook": function(touclickObject, $target) {
				...
				var loginbut = captchaButton.children("a").css({
					"width": "155px",
					"float": "left",
					"marginLeft": "6px"
				}).eq(0).css({
	...
	css["login_leftTicket-login"] = {
		"float": {
			...
			"hook": function(touclickObject, $target) {
				...
				return function() {
					var relogin = $('#relogin');
					relogin.parents('.dhtmlx_window_active').css({
						'height': '437px',
						'width': '475px'
					}); //319px
					relogin.parents('.dhtmlx_wins_body_outer').css({
						'height': '437px',
						'width': '485px'
					});
					relogin.parents('.dhtmlx_wins_body_inner').css({
						'height': '441px',
						'width': '477px'
					});
					relogin.parent().css({
					...
					var toup = $tou.parent();
					toup.parent().css({
						'padding': "44px 0 39px 64px"
					});
					toup.parent().parent().css({
						'width': '462px'
					});
					toup.siblings('li').css({
						'height': '30px'
					});
					$target.parent().parent().parent().css({
						'padding-bottom': '47px'
					});
					toup.css({
						'margin-top': '22px'
					});
					toup.siblings('.zc').add(toup).css({
						'padding-left': '65px'
					});
		...
		"inside": {
			...
			"hook": function(touclickObject, $target) {
				...
				return function() {
					...
					var captchaButton = $tou.parent();
					var ul = captchaButton.parent().css({
						"zoom": "1"
					});
					var forget_password = ul.children("li").css({
						"zoom": "1"
					}).find("input").css({
						"width": "309px"
					}).next();
					forget_password.parent().parent().css({
						"height": "49px"
					});
					...
					lis.eq(-1).add(lis.eq(-2)).add(lis.eq(-3)).css({
						"padding-left": "18px",
						"height": "20px",
						"zoom": "1"
					});
					...
					captchaButton.siblings(".txt").eq(0).css({
						"height": "4px",
						"width": "90%",
						"border-width": "2px",
						"margin": "0 auto",
					...
					captchaButton.siblings('.error').css({
						'line-height': '12px',
	...
	css["passenger_other"] = {
		...
		"inside": {
			...
			"hook": function(touclickObject, $target) {
				...
				captchaButton.children('a').css({
					'float': 'left',
					'margin-top': '172px'
				});
				var error_msg = $('#error_msg' + $target.attr('id')).css({
					'position': 'absolute',
					'left': '396px',
					'top': '116px',
					'padding-top': '8px'
